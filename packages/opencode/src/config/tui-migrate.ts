...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from mobcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const mobcode = await mobcodeFiles(input)
  for (const file of mobcode) {
...
async function mobcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "mobcode"),
    ...(await Filesystem.findUp(["mobcode.json", "mobcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "mobcode"))
  }
