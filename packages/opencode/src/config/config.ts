...
function globalConfigFile() {
  const candidates = ["mobcode.jsonc", "mobcode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mobcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "mobcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("mobcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".mobcode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["mobcode.json", "mobcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.mobcode/plugin(s)` are already local files, so ConfigPlugin.load
